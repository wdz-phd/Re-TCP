...
class SEMeasurements(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        x = self.linear1(x)
        x = self.relu1(x)
        weights = self.se_block(x)  # 计算注意力权重
        x = x * weights  # 重新加权特征
        x = self.linear2(x)
        x = self.relu2(x)
        return x
...
class AdaptiveChannelAttention(nn.Module):
    def __init__(self, num_channels, reduction_ratio=16):
        ...
        self.channel_weight = nn.Parameter(torch.randn(1, num_channels, 1, 1), requires_grad=True)
        # 学习的通道加权系数
...
class AdaptiveSpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        self.sigmoid = nn.Sigmoid()
        # 学习的空间加权系数
        self.spatial_weight = nn.Parameter(torch.randn(1, 1, 1, 1), requires_grad=True)
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x = self.sigmoid(x)
        x = x * self.spatial_weight  # 自适应加权空间注意力
        return x


# 混合注意力机制：融合视觉特征和空间上下文信息
class HybridAttention(nn.Module):
    def __init__(self, num_channels, reduction_ratio=8, kernel_size=7):
        super(HybridAttention, self).__init__()
        self.channel_attention = AdaptiveChannelAttention(num_channels, reduction_ratio)
...
# 在ResNet34中集成混合注意力机制
class ResNet34WithHybridAttention(nn.Module):
    def __init__(self):
        super(ResNet34WithHybridAttention, self).__init__()
        ...
        # 混合注意力机制：每层都应用
        self.hybrid_attention_layer1 = HybridAttention(64,8,7)
        self.hybrid_attention_layer2 = HybridAttention(128,8,7)
        self.hybrid_attention_layer3 = HybridAttention(256,8,7)
        self.hybrid_attention_layer4 = HybridAttention(512,8,7)
        # 全连接层
        self.avgpool = self.resnet34.avgpool
        self.fc = self.resnet34.fc

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # 前向传播：通过ResNet34和混合注意力块
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        x = self.layer1(x)
        x = self.hybrid_attention_layer1(x)  # 应用混合注意力
        x = self.layer2(x)
        x = self.hybrid_attention_layer2(x)  # 应用混合注意力
        x = self.layer3(x)
        x = self.hybrid_attention_layer3(x)  # 应用混合注意力
        x_layer4 = self.layer4(x)
        x_layer4 = self.hybrid_attention_layer4(x_layer4)  # 应用混合注意力
...
class TCP(nn.Module):
    def __init__(self, config):
        ...
        self.self_perception = ResNet34WithHybridAttention()
        self.self_measure = SEMeasurements()
    ...
    def forward(self, img, state, target_point):
        ...
        feature_emb, cnn_feature = self.self_perception(img)
        outputs = {}
        outputs['pred_speed'] = self.speed_branch(feature_emb)
        # measurement_feature = self.measurements(state)
        measurement_feature = self.self_measure(state)
