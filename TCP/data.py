...
class CARLA_Data(Dataset):
	...
	def __getitem__(self, index):
		...
		# if self.img_aug:
		# 	data['front_img'] = self._im_transform(augmenter(self._batch_read_number).augment_image(np.array(Image.open(self.root+self.front_img[index][0]))))
		# else:
		# 	data['front_img'] = self._im_transform(np.array(Image.open(self.root+self.front_img[index][0])))

		if self.img_aug:
			try:
				temp = np.array(Image.open(self.root + self.front_img[index][0]))
				data['front_img'] = self._im_transform(
					augmenter(self._batch_read_number).augment_image(temp))
			except:
				temp = np.array(Image.open(self.root + self.front_img[0][0]))
				data['front_img'] = self._im_transform(
					augmenter(self._batch_read_number).augment_image(temp))
		else:
			try:
				temp = np.array(Image.open(self.root + self.front_img[index][0]))
				data['front_img'] = self._im_transform(temp)
			except:
				temp = np.array(Image.open(self.root + self.front_img[0][0]))
				data['front_img'] = self._im_transform(temp)
